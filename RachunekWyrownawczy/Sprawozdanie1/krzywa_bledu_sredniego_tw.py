...
m_x2 = 0.0006386971  # Wariancja w osi X
m_y2 = 0.0003896077  # Wariancja w osi Y
m_xy = -0.0001334413  # Kowariancja
...
plt.quiver(0, 0, m_max * np.sin(phi), m_max * np.cos(phi), angles='xy', scale_units='xy', scale=1, color='r', label="Półoś większa")
plt.quiver(0, 0, m_min * np.sin(phi - np.pi / 2), m_min * np.cos(phi - np.pi / 2), angles='xy', scale_units='xy', scale=1, color='b', label="Półoś mniejsza")
...
scaling_86 = np.sqrt(4.429)  # Współczynnik dla 86% poziomu ufności
scaling_99 = np.sqrt(9.21)  # Współczynnik dla 99% poziomu ufności
