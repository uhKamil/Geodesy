import numpy as np
from wyrownanie import ocena_dokladnosci, deg_min_sec
...
Qx = ocena_dokladnosci(A, L, P)
# mw_1, mw_2 = 0.02 / np.sqrt(Qx[0][0] + Qx[1][1]), 0.02 / np.sqrt(Qx[2][2] + Qx[3][3])
# print(mw_1, mw_2)
#
# P1 = mw_1 ** -2 * np.diag([d4, d5, d6, d7, alfa3, alfa5, alfa6, k102, k103, k104])
# P2 = mw_2 ** -2 * np.diag([d4, d5, d6, d7, alfa3, alfa5, alfa6, k102, k103, k104])
# Q1, Q2 = ocena_dokladnosci(A, L, P1), ocena_dokladnosci(A, L, P2)
#
# for i in range(10):
#     print(f"Qx{np.sqrt(Qx[i][i])} = {mw_1 * np.sqrt(Q1[i][i])}")
#
# for i in range(10):
#     print(f"Qx{np.sqrt(Qx[i][i])} = {mw_2 * np.sqrt(Q2[i][i])}")